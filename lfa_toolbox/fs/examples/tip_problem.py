...
from lfa_toolbox.fs.core.fis.fis import OR_max, MIN, FIS, COA_func
...
def tip_problem():
    ...
    r1 = FuzzyRule(
        ants=[Antecedent(lv_quality, "poor"), Antecedent(lv_service, "poor")],
        ant_act_func=OR_max,
        cons=[Consequent(lv_tip, "low")],
        impl_func=MIN,
    )

    r2 = FuzzyRule(
        ants=[Antecedent(lv_service, "average")],
        ant_act_func=OR_max,
        cons=[Consequent(lv_tip, "medium")],
        impl_func=MIN,
    )

    r3 = FuzzyRule(
        ants=[Antecedent(lv_service, "good"), Antecedent(lv_quality, "good")],
        ant_act_func=OR_max,
        cons=[Consequent(lv_tip, "high")],
        impl_func=MIN,
    )

    fis = FIS(rules=[r1, r2, r3], aggr_func=np.max, defuzz_func=COA_func)
