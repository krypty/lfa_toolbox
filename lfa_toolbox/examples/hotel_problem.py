...
from lfa_toolbox.fs.core.fis.fis import FIS, OR_max, COA_func, MIN
...
def hotel_problem():
    ...
    r1 = FuzzyRule(
        ants=[Antecedent(lv_service, "good"), Antecedent(lv_service, "very good")],
        ant_act_func=OR_max,
        cons=[Consequent(lv_recommendation, "high")],
        impl_func=MIN,
    )

    print(r1)

    fis = FIS(aggr_func=np.max, defuzz_func=COA_func, rules=[r1])
