...
from lfa_toolbox.fs.core.fis.fis import AND_min, OR_max, MIN, COA_func, FIS
...
def resort_problem():
    ...
    r1 = FuzzyRule(
        ants=[Antecedent(lv_temperature, "hot"), Antecedent(lv_sunshine, "sunny")],
        ant_act_func=OR_max,
        cons=[Consequent(lv_tourists, "high")],
        impl_func=MIN,
    )
    ...
    r2 = FuzzyRule(
        ...
        cons=[Consequent(lv_tourists, "medium")],
        impl_func=MIN,
    )

    r3 = FuzzyRule(
        ants=[Antecedent(lv_temperature, "cold"), Antecedent(lv_sunshine, "cloudy")],
        ant_act_func=OR_max,
        cons=[Consequent(lv_tourists, "low")],
        impl_func=MIN,
    )

    # fis = SingletonFIS(
    fis = FIS(aggr_func=np.max, defuzz_func=COA_func, rules=[r1, r2, r3])
